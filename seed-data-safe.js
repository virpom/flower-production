...
async function seedDatabase() {
  ...
  try {
    ...
    // Создаем подкатегории с уникальными slug
    ...
    const subcategories = [
      { 
        ...
        name: 'Букеты из роз', 
        slug: 'bukety-iz-roz', 
        categoryId: null, // Заполним позже
        description: 'Роскошные букеты из роз', 
        image: '', 
        isActive: true, 
        createdAt: new Date(), 
        updatedAt: new Date() 
    ...
    // Создаем категории после подкатегорий
    ...
    // Обновляем categoryId в подкатегориях
    subcategories[0].categoryId = categories[0]._id;
    subcategories[1].categoryId = categories[1]._id;
